...
//Slider searching section for html in all page
...
let busqueda = document.getElementById("box-search");

searching.map((x) => {
    busqueda.innerHTML += `
    <li><a href="${x.link}"><i class="fa-solid fa-magnifying-glass"></i>${x.name}</a></li>
    `;
})
...
window.addEventListener('load', () => {
    ...
    //Adding listener from title link

    document.querySelectorAll('.grid .item .box-hover h3').forEach( (element) => {
...
function SendFormHome() {
    ...
    document.elementById("entry.764054735").focus();
        ...
        location.href = "../contacto/sendmail-home.html#sendmail";
        if(form=="success") {
            swal('Mensaje enviado','Sus datos han sido enviados exitosamente','success');
        } else {
            swal('Error','El mensaje no se pudo enviar. Inténtelo de nuevo','error');
